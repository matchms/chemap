...
class MAP4neo:
    ...
    def visualize(
        ...
        """
        Visualize the molecules using t-SNE.
        ...
        number_of_threads : Optional[int], default=None
            Number of threads for t-SNE.
        metric : str, default="approximated_jaccard"
            The distance metric.
        preliminarly_reduce_with_pca : bool, default=False
            If True, first reduce dimensionality with PCA.
        ...
        fingerprints: np.ndarray = self.calculate_many(
            molecules,
        ...
        # Determine the most common labels (up to 20 colors)
        label_counter = Counter(labels)
        if len(label_counter) > 20:
            most_common_labels: List[str] = [
                label for label, _ in label_counter.most_common(19)
            ]
            most_common_labels.append("Other")
        else:
            most_common_labels = [label for label, _ in label_counter.most_common()]
